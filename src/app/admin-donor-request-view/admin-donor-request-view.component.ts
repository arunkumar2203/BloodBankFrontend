...
export class AdminDonorRequestViewComponent implements OnInit {
  ...
  public onAcceptRequest(request:any){
    request.status=1
    this.donorRequest=new DonorRequest(request.id,request.username,request.patientName,request.patientAge,request.reason,request.bloodGroup,request.units,request.status,request.fileName)
